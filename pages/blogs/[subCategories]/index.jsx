                            ...
                            <div>
                                <div className="flex justify-between">
                                    <div>
                                        ...
                                        <BlogHeader category={'Blogs'} title={param?.subCategories} subCategory={param?.subCategories} />
                                    </div>
                                    <p className="mb-8">Total {BLOGS.length} results found.</p>
                                </div>
                                <div className="grid grid-cols-1 lg:grid-cols-4 gap-[20px]">
                                    {BLOGS && BLOGS.length > 0 && BLOGS.map(({title, desc, link, imgUrl}) => {
                                        return <BlogCard title={title} category={param?.subCategories} desc={desc} link={`/blogs/${param?.subCategories}/${link}`} imgUrl={imgUrl} />
                                    })}
                                </div>
                            </div>
                        </div>
